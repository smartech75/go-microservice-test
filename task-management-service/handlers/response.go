...
func SuccessItemsRespond(fields interface{}, modelType string, writer http.ResponseWriter) {
	_, err := json.Marshal(fields)
	type data struct {
		Data    interface{} `json:"data"`
		Message string      `json:"message"`
	...
	temp := &data{Data: fields, Message: "success"}
	if err != nil {
		color.Red("Marshal Data Failed...", modelType)
		ServerErrResponse(err.Error(), writer)
	}
	...
	case "User":
		temp.Data = fields.([]*models.User)
	case "Task":
		temp.Data = fields.([]*models.Task)
	default:
		color.Red("Invalid Model Type...", modelType)
	}
